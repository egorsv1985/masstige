...
?>
<div class="form-contaner px-4 py-4 text-center">
	...
	<?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>"><?endif?>
				...
				?>
						...
						switch ($INPUT_TYPE):
							...
							case "T":
								...
								?>
						<textarea class="form-input d-block w-100 mb-3" cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
								<?
								}
							break;
							...
							case "N":
								...
								?>
								<input <?=(in_array($propertyID, $arResult["PROPERTY_REQUIRED"]) ? 'required="required" ' : '')?>class="form-input d-block w-100 mb-3" type="text" placeholder="<?=$PLACEHOLDER;?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]; ?>" value="<?=$value?>"><?
		...
		<?endif?>
					...
					<div class="polity mt-3 mb-2">
						Нажимая на кнопку, вы соглашаетесь с <a href="/polity/" target="_blank">политикой конфиденциальности</a>
					</div>
